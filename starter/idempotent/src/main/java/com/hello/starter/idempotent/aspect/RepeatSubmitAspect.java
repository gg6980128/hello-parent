package com.hello.starter.idempotent.aspect;
...
public class RepeatSubmitAspect {
    ...
    public void doBefore(JoinPoint point, RepeatSubmit repeatSubmit) throws Throwable {
        ...
        if (interval < 1000) {
            throw new BusinessException(500, "重复提交间隔时间不能小于'1'秒");
        }
        ...
        // 唯一标识（指定key + url + 消息头）
        String cacheRepeatKey = "repeat-submit" + url + submitKey;
        if (RedisUtils.setObjectIfAbsent(cacheRepeatKey, "", Duration.ofMillis(interval))) {
            KEY_CACHE.set(cacheRepeatKey);
        } else {
            String message = repeatSubmit.message();
            if (StrUtil.startWith(message, "{") && StrUtil.endWith(message, "}")) {
                message = MessageResourceUtils.format(StrUtil.sub(message, 1, message.length() - 1));
            }
