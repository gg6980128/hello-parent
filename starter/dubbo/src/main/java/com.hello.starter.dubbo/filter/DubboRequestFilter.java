package com.hello.starter.dubbo.filter;
...
public class DubboRequestFilter implements Filter {
    ...
    public Result invoke(Invoker<?> invoker, Invocation invocation) throws RpcException {
        ...
        // 根据日志级别输出不同详细程度的日志信息
        if (RequestLog.INFO.equals(properties.getLogLevel())) {
        ...
        if (result.hasException() && !invoker.getInterface().equals(GenericService.class)) {
            ...
            } else if (RequestLog.FULL.equals(properties.getLogLevel())) {
                log.info("DUBBO - 服务响应: {},SpendTime=[{}ms],Response={}", baselog, elapsed, JacksonUtils.toJsonString(new Object[]{result.getValue()}));
            }
        }
        return result;
    }
